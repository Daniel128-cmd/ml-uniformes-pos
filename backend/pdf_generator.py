...
def generate_orden_consolidada(db_path: str) -> str:
    """Orden de fabricación: agrupa por colegio → prenda → talla, suma cantidades. Sin precios."""
    delivery_target = _add_business_days(datetime.now(), 15).strftime("%d/%m/%Y")
    today_str = datetime.now().strftime("%d/%m/%Y")
    ...
    cur.execute("""
        ...
        JOIN products p   ON ri.product_id = p.id
        WHERE r.deleted_at IS NULL
        GROUP BY i.name, p.name, ri.size_range, ri.exact_size
        ORDER BY i.name, p.name, ri.size_range, ri.exact_size
    """)
    rows = cur.fetchall()
    conn.close()

    filename = os.path.join(BASE_DIR,
                            f"orden_consolidada_{datetime.now().strftime('%Y%m%d')}.pdf")
    doc = SimpleDocTemplate(filename, pagesize=letter,
                            leftMargin=0.6*inch, rightMargin=0.6*inch,
                            topMargin=0.5*inch, bottomMargin=0.5*inch)
    story = []
    ...
    if not rows:
        ...
        grand_total = 0
        for inst_name, items in sorted(grouped.items()):
            _school_header(story, inst_name)
            data = [["Prenda", "Talla", "Cant."]]
            inst_total = 0
            for it in items:
                talla_parts = []
                if it["size_range"]: talla_parts.append(it["size_range"])
                if it["exact_size"]:  talla_parts.append(it["exact_size"])
                talla = it["exact_size"] or it["size_range"] or "Única"
                qty = int(it["total_qty"])
                data.append([it["product"], talla, str(qty)])
                inst_total += qty
            data.append(["", "TOTAL PRENDAS:", str(inst_total)])
            grand_total += inst_total

            t = Table(data, colWidths=[3.2*inch, 2.3*inch, 1.0*inch])
            t.setStyle(TableStyle([
                ('BACKGROUND',    (0, 0), (-1, 0), VINO),
                ('TEXTCOLOR',     (0, 0), (-1, 0), BEIGE),
                ('FONTNAME',      (0, 0), (-1, 0), 'Helvetica-Bold'),
                ...
                ('TOPPADDING',    (0, 1), (-1, -2), 6),
                ('BOTTOMPADDING', (0, 1), (-1, -2), 6),
                ('BACKGROUND',    (0, -1), (-1, -1), VINO_DARK),
                ('TEXTCOLOR',     (0, -1), (-1, -1), BEIGE),
                ('FONTNAME',      (0, -1), (-1, -1), 'Helvetica-Bold'),
                ('FONTSIZE',      (0, -1), (-1, -1), 9),
        ...
        story.append(HRFlowable(color=VINO, thickness=2))
        story.append(Spacer(1, 0.1*inch))
        summ_data = [["Concepto", "Total"],
                     ["TOTAL GENERAL DE PRENDAS A PRODUCIR", str(grand_total)]]
        summ_t = Table(summ_data, colWidths=[5.0*inch, 1.5*inch])
        summ_t.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), VINO),
            ('TEXTCOLOR',  (0, 0), (-1, 0), BEIGE),
    ...
    _footer(story, f"ML Uniformes — Orden de Producción Consolidada — {_date_label()}")
    doc.build(story)
    return filename
